...
export function WebhookListItem({ webhook }: WebhookListItem) {
  ...
  return (
    <div className="relative group rounded-lg transition-colors duration-150 hover:bg-zinc-700/30 ">
      <div className="px-4 py-2.5 flex items-start gap-3">
        ...
        <button tabIndex={-1} disabled={isPending} onClick={() => deleteMutation(webhook.id)} className="absolute top-1/2 right-4 -translate-y-1/2 disabled:opacity-50 disabled:cursor-not-allowed opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100">
